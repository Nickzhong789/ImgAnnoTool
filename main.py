...
class AnnotionToolMainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.savaAnnoBtn = QtWidgets.QPushButton(self)
        self.savaAnnoBtn.setText("save")
        self.savaAnnoBtn.setShortcut('s')
        ...
        self.buttonsLayout.addWidget(self.savaAnnoBtn)
    ...
    def connectEvent(self):
        ...
        self.savaAnnoBtn.clicked.connect(self.saveAnnoBtn_click)

    def getKpWeight(self):
        weight, ok = QInputDialog.getText(self, "Input Weight", "请输入权值")
        if ok:
            print(weight)
            return weight

    def saveAnnoBtn_click(self):
    ...
    def openImage(self):
        ...
        anno = str("./annotion/" + file.split("/")[-1].split(".")[0] + "_anno.json")
    ...
    def keypointListView_click(self):
        print('kpoint')
        k_list = list(self.keypointListView.currentIndex().data().split(':')[1])
    ...
    def predefine_classListView_click(self):
        self.refreshPredefineKeypointListView(
            int(self.predefine_classListView.model().data(self.predefine_classListView.currentIndex(), 0).split(':\t')[
                    0])
        )
    ...
    def predefine_keypointListView_click(self):
        ...
        if self.canvas.nextbegin == None:
            return
        self.predefine_keypointListView.setCurrentIndex(
            self.predefine_keypointListView.model().createIndex(self.canvas.nextbegin, 0))

    def keyReleaseEvent(self, e):
        # if QtCore.Qt.Key_0 <= e.key() <= QtCore.Qt.Key_9:
        if QtCore.Qt.Key_0 == e.key():
            self.refreshPredefineKeypointListView(
            ...
            if self.predefine_keypointListView.model() == None:
                return
            i = self.canvas.findNextKeypointIndex((self.predefine_keypointListView.currentIndex().row() + 1) %
                                                  self.predefine_keypointListView.model().rowCount())
            if i == None:
                i = 0
    ...
    def addKeypoint(self, ev, kid, cls, ppicker, picker, weight):
        self.kpointLists[ppicker].append(
            '{}:{}'.format(AnnotionConfigure.instance().getKeypointConfigByClassId(cls)[kid][1],
                           [picker, int(ev.xdata), int(ev.ydata), weight]))
        self.keypointListModel.setStringList(self.kpointLists[ppicker])
        self.keypointListView.setModel(self.keypointListModel)

    def addKp(self, kx, ky, kid, cls, ppicker, picker, weight):
        self.kpointLists[ppicker].append(
            '{}:{}'.format(AnnotionConfigure.instance().getKeypointConfigByClassId(cls)[kid][1],
                           [picker, kx, ky, weight]))
        self.keypointListModel.setStringList(self.kpointLists[ppicker])
    ...
    def deleteKeypoint(self, kid, cls, ppicker, picker):
        print(self.kpointLists[ppicker])
        print(self.kpointLists[ppicker][picker-1].split(':')[-1])
        self.kpointLists[ppicker].remove(
            '{}:{}'.format(AnnotionConfigure.instance().getKeypointConfigByClassId(cls)[kid][1],
                           self.kpointLists[ppicker][picker-1].split(':')[-1]))
        self.keypointListModel.setStringList(self.kpointLists[ppicker])
        self.keypointListView.setModel(self.keypointListModel)
